...
class QLearner(object):
    ...
    def step_env(self):
        ...
        # START OF YOUR CODE
        # ----------------------------------------------------------------------
        index = self.replay_buffer.store_frame(self.last_obs)
        
        if np.random.uniform() < self.exploration.value(self.t) or not self.model_initialized:
            act = np.random.randint(0, self.num_actions)
        
        else:
            obs = self.replay_buffer.encode_recent_observation()
            feed = {self.obs_t_ph : [obs]}
            act = self.session.run(self.best_act, feed_dict=feed)[0]
        ...
        self.replay_buffer.store_effect(index, act, reward, done)
