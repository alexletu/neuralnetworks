...
class AttentionQKV(Model):
    ...
    def call(self, queries, keys, values, mask=None):
        ...
        similarity = tf.linalg.matmul(queries, keys, transpose_b=True) / tf.math.sqrt(key_dim) # Compute the similarity according to the QKV formula
        ...
        weights = tf.keras.activations.softmax(masked_similarity, axis=-1) # Turn the similarity into a normalized output
